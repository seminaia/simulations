"""
MERGED USABLE VERSION
This is the full homework script with the completed Problems 1–10 sections.
...
def optimize_outer_PI(cascade, Kc2, t, ysp_input, d_input, initial_guess=(0.2, 10.0)):
    ...
    def objective(log_params):
        ...
        try:
            ...
            if not np.all(np.isfinite(y_sim)) or np.max(np.abs(y_sim)) > 1e8:
                return 1e12
...
>>>>>>> 8249be9fdc69973327289a9454225d03867988ce
    ...
    return Kc_opt, tauI_opt, float(result.fun)
...
def lambda_pi_for_fopdt(K, tau, theta, lam=None):
    """
    Simple IMC/lambda PI tuning for self-regulating FOPDT:
    ...
    Kc = tau / (K * (lam + theta))
    tauI = tau
    return float(Kc), float(tauI), float(lam)
...
def run_problems_1_to_3():
    ...
    Kc2_no = 1.0
    P_no, tauI_no, _ = optimize_outer_PI(
        cascade=False,
        Kc2=Kc2_no,
        t=tvals,
        ysp_input=step_off,
        d_input=step_on,
        initial_guess=(0.2, 10.0),
    )
    I_no = 1 / tauI_no
    ...
    Kc2_cas = 0.4
    P_cas, tauI_cas, _ = optimize_outer_PI(
        cascade=True,
        Kc2=Kc2_cas,
        t=tvals,
        ysp_input=step_off,
...
<<<<<<< HEAD
    ...
    Kc22, tauI22, lam22 = lambda_pi_for_fopdt(6, 10, 3)
