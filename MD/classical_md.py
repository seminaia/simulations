...
H model     : H⁺ (proton, charge +1) — each H⁺ paired with one extra F⁻
              to maintain charge neutrality required by Ewald summation.
              Physically models HF dissolved/dissociated in the melt.
H-F params  : approximate Tosi-Fumi scaling (verify against ab initio
              for quantitative work, e.g. Salanne/Madden MSR potentials)
Workflow    : load equilibrated structure → insert H⁺/F⁻ pairs →
...
N_H_PAIRS       = 20        # number of H⁺ / H⁺ pairs to insert
...
print(f"  Atoms before H insertion : {len(atoms)}")
...
print(f"\nInserting {N_H_PAIRS} H⁺/H⁺ pairs (charge-neutral H2 dissolution model)...")
...
for _ in range(N_H_PAIRS):
    h_pos = random_interstitial(atoms, MIN_INSERT_DIST)
    atoms.append(Atom('H', position=h_pos))
    h2_pos = random_interstitial(atoms, MIN_INSERT_DIST)
    atoms.append(Atom('H', position=h2_pos))
    n_inserted += 1

print(f"  Inserted {n_inserted} H⁺ and {n_inserted} H⁺ (total {2*n_inserted} atoms)")
print(f"  Total atoms after insertion : {len(atoms)}")
sym_counts = {s: list(atoms.get_chemical_symbols()).count(s)
              for s in sorted(set(atoms.get_chemical_symbols()))}
print(f"  Composition : {sym_counts}")

# ── Ionic charges ─────────────────────────────────────────────────────────────
charge_map = {'Li': 1.0, 'Be': 2.0, 'F': -1.0, 'H': 1.0}
...
# ── Buckingham pair coefficients  (specorder: Li=1, Be=2, F=3, H=4) ──────────
#
#  Pair    A (eV)     ρ (Å)     C (eV·Å⁶)   Source
#  Li–Li    0.0       1.0        0.0         Coulomb only
...
#  F–H    340.0     0.24000      0.0         approx. Tosi-Fumi scaling for H⁺
#  H–H      0.0       1.0        0.0         Coulomb only
pair_coeff = [
...
print("\nMinimising energy to relax inserted H/H positions (fmax=0.05 eV/Å)...")
minimiser = BFGS(atoms, logfile='LiF_BeF2_H_minimisation.log')
...
dyn.attach(MDLogger(dyn, atoms, "LiF_BeF2_H_md.log"), interval=LOG_INTERVAL)
...
for i in range(n_blocks):
    ...
    if (i + 1) % 10 == 0:
        print(f"  block {i+1:4d}/{n_blocks}  t={time_ps[-1]:.3f} ps  "
              ...
              f"Etot={(epot_list[-1]+ekin_list[-1])/len(atoms):.4f} eV/atom")
...
lines = [
    f"System : LiF·BeF2 + {N_H_PAIRS} H2  ({len(atoms)} atoms)",
    f"T      : {TEMPERATURE} K",
