...
matplotlib.use('TkAgg')   # <-- FIX: avoids FT_Render_Glyph crash
...
# ---- FIX: also scale the label offsets (column 5) by 4.184 ----
for arr in [oxss, oxls, oxgs, oxsl, oxll, oxgl, oxsg, oxlg, oxgg]:
    if arr.shape[1] > 5:
...
for arr in [niss, nils, nigs, nisl, nill, nigl, nisg, nilg, nigg]:
    if arr.shape[1] > 5:
        arr[:, 5] = arr[:, 5].astype(float) * 4.184
...
for arr in [flss, flls, flgs, flsl, flll, flgl, flsg, fllg, flgg]:
    if arr.shape[1] > 5:
        arr[:, 5] = arr[:, 5].astype(float) * 4.184
...
for arr in [clss, clls, clgs, clsl, clll, clgl, clsg, cllg, clgg]:
    if arr.shape[1] > 5:
        arr[:, 5] = arr[:, 5].astype(float) * 4.184
...
for row in oxss:
    a1.text(float(row[0]) - 50, float(row[2]) + float(row[5]),   # <-- increased horizontal offset
            row[4],
...
# Add text labels for each reaction 
for row in cass:
    a2.text(float(row[0]) - 50, float(row[2]) + float(row[5]),   # <-- increased horizontal offset
            row[4],
            horizontalalignment='right',
            verticalalignment='center',
            fontsize=8)
...
# Title and axis titles – fixed raw strings
