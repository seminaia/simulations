...
import numpy as np
import matplotlib
matplotlib.use('TkAgg')   # avoids raster overflow
...
# ----------------------------------------------------------------------
# Data – original values in K and kcal per mole of reacting gas
# (copied verbatim from your script, then converted later)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# Unit conversion: K → °C, kcal → kJ (FIXED)
# ----------------------------------------------------------------------
def convert_units(*arrays):
    for arr in arrays:
        if arr.size == 0:
            continue
        # Extract first four columns as floats, convert, then assign back
...
# ----------------------------------------------------------------------
# Helper to collect all phase arrays for an anion
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# Plotting function for one anion
# ----------------------------------------------------------------------
def plot_anion(ax, anion_dict, color, title, ylabel, xlabel='Temperature (°C)'):
    ...
    for phase, arr in anion_dict.items():
        ...
        for row in arr:
            # Ensure numeric values (fallback if still strings)
            T0 = float(row[0])
            T1 = float(row[1])
            G0 = float(row[2])
            G1 = float(row[3])
            ...
            ax.plot([T0, T1], [G0, G1],
                    color=color, ls=style['ls'], alpha=style['alpha'],
...
def save_anion_figure(anion_dict, color, name, ylabel):
    ...
    plot_anion(ax, anion_dict, color, title=f'{name.capitalize()}', ylabel=ylabel)
    plt.tight_layout()
