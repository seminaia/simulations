...
- `G0`, `G1`: segment endpoint ΔG values in **kcal/mol** of the family reference gas (or per mol C for carbides) — **except `carbides`, whose values are already in kJ/mol C** (transcribed directly from Coltters 1985)
...
`ellingham.py` keeps these source units in the workbook and still performs the existing one-time K→°C conversion after loading, plus kcal→kJ for every family *except* `carbides` (`convert_units(..., gibbs_already_kj=True)` skips that second scaling there since the sheet is already in kJ). Family sheets are loaded lazily: the script only parses the worksheet(s) requested by `--families` instead of reading all 11 up front.
